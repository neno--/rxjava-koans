...
public class lessonB_MapAndFlatMapAndBasicOperators {
  ...
  /**
   * Understanding what flatMap() does is a major awakening on the seeker's path to rx enlightenment. We will use non-lambda syntax here to help
   * illustrate what the return types are in this use case for flatmap. For this experiment, we will be going to the carnival. Because we spent our
   * money unwisely at the carnival ($25 dollars on the Dunk Tank), we are left only with 5$. We still need to eat though. Our goal - check the
   * available food options and get a filtered list of things under 5$.
   */
  ...
  public void _3_theReduceOperatorAccumulatesValuesAndEmitsTheResult() {
    ...
    assertThat(testSubscriber.getOnNextEvents().get(0)).isEqualTo(168 + 234 + 192 + 142 + 114);
  }
  ...
  /**
   * A great feature of RxJava is that we can chain actions together to achieve more functionality. In this example we have one Observable and we
   * perform two actions on the data it emits. Lets build two Strings by concatenating some integers.
   */
  @Test
  ...
  /**
   * Instead of just using events as input to actions (for example summing them), we can transform the events themselves. We'll use the map() function
   * for this. Lets take some text and map it to all lowercase. The key to making this work is to return the same variable that comes into the
   * action.
   */
  @Test
